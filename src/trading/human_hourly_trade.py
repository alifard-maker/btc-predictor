...
def _clamp_stake_usd(stake: float) -> float:
  return round(max(0.50, min(100.0, float(stake))), 2)
...
def settings_from_cfg(cfg: dict[str, Any] | None, store: HumanTradeStore) -> HumanTradeSettings:
  ...
  return HumanTradeSettings(
    ...
    max_stake_per_entry_usd=_clamp_stake_usd(
...
def apply_human_settings_body(
  ...
  *,
  ...
  updated = HumanTradeSettings(
    mode=mode,
    max_stake_per_entry_usd=_clamp_stake_usd(
      float(body.get("max_stake_per_entry_usd", current.max_stake_per_entry_usd)),
    ),
    paper_bankroll_initial_usd=float(
